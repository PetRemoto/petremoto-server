...
router.get('/', function (req, res) {
    ...
    User.findById(req.query.id).populate('dispensers').populate('requests').populate('friends').exec(function (err, users) {
        if (!err) {
...
//
//    User.find(rules).populate('dispensers').populate('requests').populate('friends').exec(function (err, users) {
//        if (!err) {
//
//            var result = {};
//            result.users = users;
...
router.get('/all', function (req, res) {
    ...
    User.find({}).populate('dispensers').populate('requests').populate('friends').exec(function (err, result) {
        if (!err) {
            res.send(result);
        }
    });
